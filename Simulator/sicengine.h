...
void GetIR (ADDRESS Addr, char * Inst)
{
  ...
  char Build[12];
    ...
    switch (Ops[OpCode].FORM) {
        case 1:                 /* No operands */
                util::m_sprintf(Inst, "%6s", Ops[OpCode].OP);
                break;
        case 2:                 /* Register-Register */
        R1 = IR[1] >> 4 & 0x7;
        R2 = IR[1] & 0x7;
                util::m_sprintf(Inst, "%6s  %c,%c", Ops[OpCode].OP, REG[R1], REG[R2]);
                break;
        case 3:                 /* Format 3 & 4 and SIC */
                if (N == 0 && I == 0) {         /* SIC */
            Oper = (IR[1] & 0x7f) * 256 + IR[2];
            util::m_sprintf(Inst, "%6s  [%04lx]%s", Ops[OpCode].OP, Oper,
                                (X == 1) ? ",X" : " ");
                    break;
                }
                Build[0] = '\0';
                if (!(N == 1 && I == 1) )
                    if (N == 1)
                        ...
                        util::m_strcat(Build, "#");
                if (B == 1)
                    util::m_strcat(Build, "(B)");
                if (P == 1)
                    util::m_strcat(Build, "(PC)");
                Oper = (IR[1] & 0xf) * 256 + IR[2];
                if (E == 1) {                   /* extended form */
                    GetMem (Addr + 3, IR, 0);
                    Oper = Oper * 256 + (IR[0] & 0xff);
            util::m_sprintf(Inst, "+%6s  %s%05lx", Ops[OpCode].OP, Build,
                                        Oper);
                } else
            util::m_sprintf(Inst, "%6s  %s%03lx", Ops[OpCode].OP, Build,
                                        Oper);
                if (X == 1)
                    util::m_strcat(Inst, ",X");
    }
}
               ...
               WORD data, ADDRESS *opaddr)
{
  ...
  int Devcode;            /* holds I/O device number */
      ...
      if (opcode == 216) {  /* RD */
          ...
          if (!ERROR)
              if (!Init[Devcode]) {
                  if ((Dev[Devcode] = util::m_fopen(SICFile[Devcode],"r")) == NULL) {
      ...
      if (opcode == 220) {   /* WD */
          ...
          if (!ERROR) {
              if (!Init[Devcode]) {
                  if ((Dev[Devcode] = util::m_fopen(SICFile[Devcode],"w")) == NULL) {
                      printf("cannot open file %s\n", SICFile[Devcode]);
                      exit(1);
                  }
                  Init[Devcode] = TRUE;
...
void SICStart()
{
  ...
  BOOLEAN err1;
     ...
     err1 = FALSE;
     if ((DevBoot = util::m_fopen("dev00","r")) == NULL) {
...
void SICRun(BOOLEAN SingleStep){
	SICRun(&PC,SingleStep);
}
...
void SICInit()
{
...
#if 0
     if ((Log = util::m_fopen("log","w")) == NULL) {
  ...
  /* +++++ initialization for non-ascii character sets goes here +++++ */
     ...
     for (i = 0; i < 3; i++)        /* initialize status word */
         Status[i] = 0;
     Msg[0] = util::m_strdup(" ");
     Msg[1] = util::m_strdup("Division by zero");
     Msg[2] = util::m_strdup("Integer overflow");
     Msg[3] = util::m_strdup("Invalid address");
     Msg[4] = util::m_strdup("Invalid register");
     Msg[5] = util::m_strdup("Unsupported instruction");
     Msg[6] = util::m_strdup("Illegal instruction");
     Msg[7] = util::m_strdup("Illegal addressing mode");
     Msg[8] = util::m_strdup("Immediate not allowed");
     Msg[9] = util::m_strdup("Unsupported I/O device");
     Msg[10] = util::m_strdup("I/O device not ready");
     Msg[11] = util::m_strdup("Device not open for read");
     Msg[12] = util::m_strdup("Device not open for write");
     Msg[13] = util::m_strdup("End of file reached");
     for (i = 14; i < 16; i++)
         Msg[i] = util::m_strdup(" ");
} /* SICInit */
